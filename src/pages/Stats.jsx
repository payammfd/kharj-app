...
export default function Stats({ plan }) {
  ...
  useEffect(()=>{
    if(!plan)return
    supabase.from('transactions').select('*').eq('plan_id',plan.id).eq('jy',vm.jy).eq('jm',vm.jm)
  ...
  function buildDonut(){
    ...
    return sorted.map(([cat,amt])=>{
      const pct=amt/totalExp,dash=pct*circ
      const seg={cat,amt,pct,dash,offset,color:CHex[cat]||'#9090A0'}
      offset+=dash;return seg
    })
  }
  const segments=buildDonut()
  const isMin=vm.jy===1405&&vm.jm===1,isCur=vm.jy===today[0]&&vm.jm===today[1]
  function prev(){if(isMin)return;setVm(p=>p.jm===1?{jy:p.jy-1,jm:12}:{jy:p.jy,jm:p.jm-1})}
  function next(){if(isCur)return;setVm(p=>p.jm===12?{jy:p.jy+1,jm:1}:{jy:p.jy,jm:p.jm+1})}

  return (
    <div className={s.page}>
      <div className={s.glow}/>
      <header className={s.hdr}><h1 className={s.title}>آمار</h1></header>
      <div className={s.monthNav}>
        <button className={s.navBtn} onClick={next} disabled={isCur}>‹</button>
        <div className={s.monthInfo}><div className={s.monthName}>{MONTHS[vm.jm-1]}</div><div className={s.monthYear}>{toFa(vm.jy)}</div></div>
        <button className={s.navBtn} onClick={prev} disabled={isMin}>›</button>
      </div>
      ...
      {sorted.length===0?(
        ...
        <div className={s.donutWrap}>
          <svg width="180" height="180" viewBox="0 0 180 180">
            ...
            {segments.map((seg,i)=>(
              <circle key={i} cx="90" cy="90" r="70" fill="none" stroke={seg.color} strokeWidth="28"
                strokeDasharray={`${seg.dash} ${2*Math.PI*70-seg.dash}`}
                strokeDashoffset={-seg.offset} transform="rotate(-90 90 90)" strokeLinecap="round"/>
            ))}
            <text x="90" y="85" textAnchor="middle" fill="rgba(255,255,255,0.4)" fontSize="10" fontFamily="Vazirmatn">هزینه کل</text>
            <text x="90" y="102" textAnchor="middle" fill="white" fontSize="11" fontWeight="700" fontFamily="Vazirmatn">{toFa(Math.round(totalExp/1000000*10)/10)} M</text>
          </svg>
        ...
        <div className={s.barList}>
          {sorted.map(([cat,amt])=>(
            <div key={cat} className={s.barItem}>
              <div className={s.barHeader}><span className={s.barCat}>{cat}</span><span className={s.barAmt}>{fmtAmount(amt)}</span></div>
              <div className={s.barTrack}><div className={s.barFill} style={{width:`${(amt/max)*100}%`,background:CHex[cat]||'#9090A0'}}/></div>
            </div>
          ))}
        </div>
      ):(
        <div className={s.tableWrap}>
          {sorted.map(([cat,amt])=>(
            <div key={cat} className={s.tableRow}>
              <div className={s.tableDot} style={{background:CHex[cat]||'#9090A0'}}/>
          ...
          <div className={s.tableTotal}><span>جمع کل</span><span>{fmtAmount(totalExp)}</span></div>
        </div>
      )}
    </div>
  )
