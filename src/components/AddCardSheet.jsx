      ...
      <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'12px'}}>
        <div className={styles.bankGrid}>
          {Object.entries(BANKS).map(([key,b])=>(
            <div key={key} className={`${styles.bankOpt} ${bank===key?styles.sel:''}`} onClick={()=>setBank(key)}>
              <div className={styles.bankDot} style={{background:b.gradient.split(',').pop()?.trim()?.replace(')','')+')'||'#555'}}/>
              {b.label}
            </div>
          ))}
        </div>
        <div className={styles.field}>
          ...
          <input className={styles.inpLtr} type="text" value={num} onChange={e=>setNum(fmtNum(e.target.value))} placeholder="xxxx  xxxx  xxxx  xxxx" maxLength={22}/>
        </div>
        <div className={styles.inpRow2}>
          <div className={styles.field}>
            <label className={styles.lbl}>تاریخ انقضا</label>
            <input className={styles.inpLtr} type="text" value={expiry} onChange={e=>setExpiry(fmtExp(e.target.value))} placeholder="MM/YY" maxLength={5}/>
          </div>
          <div className={styles.field}>
            <label className={styles.lbl}>CVV2</label>
            <input className={styles.inpLtr} type="password" value={cvv} onChange={e=>setCvv(e.target.value)} placeholder="•••" maxLength={4}/>
          </div>
        </div>
        <div className={styles.field}>
          <label className={styles.lbl}>نام صاحب کارت</label>
          <input className={styles.inpLtr} type="text" value={holder} onChange={e=>setHolder(e.target.value)} placeholder="FIRST LAST"/>
        </div>
        <div className={styles.field}>
